'''
#ChatGPT
POC code to take a JSON object containing our "guess" and encode it
+ embed the CRC value and then encrypt it using a provided public key

This is part of the final "checker" in which the user must select a combination of three cards
they believe is the winner. They encrypt it with 

MAKE SURE THE PRIVATE/PUBLIC KEY PAIR ARE DIFFERENT THAN THE CLUE PAIR!

Our actual submission is quite a bit different than the example
and will require some fine tuning as it will be a combination of three discrete JSON objects
and order matters. We'll have to sort it first, then concat all 3 JSON objects
and then submit to the submission function.
